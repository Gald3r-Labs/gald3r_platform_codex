...
<h1 align="center">Gald3r for Codex CLI</h1>

<p align="center">
  The <strong>Codex CLI edition</strong> of Gald3r -- file-based memory, task management, and
  agent orchestration that runs <strong>inside Codex CLI</strong>. Part of the Gald3r framework
...
This is the **Codex CLI distribution** of Gald3r. The [`codex/`](./codex/) folder is a
complete, ready-to-deploy gald3r setup tuned for Codex CLI -- drop its contents into your
...
cp -r gald3r_platform_codex/codex/. /path/to/your/project/
```

Then open the project in Codex CLI and type `@g-status` to confirm gald3r loaded.

> The `codex/` payload is **additive** -- it drops in gald3r's config + `.gald3r/` state and
